...
class Mapper internal constructor(
    private val sqlClient: SqlClient,
    private val converterRegistry: ImmutableConverterRegistry
) {

    private class ParameterAndConvert(
        val parameter: KParameter,
        val converter: org.mariella.persistence.database.Converter<*>
    )
    ...
    private fun <T : Any> RowSet<Row>.mapRowsToObjects(
        ...
        val converters = columnDescriptors().map { columnDescriptor ->
            val parameter = primaryConstructor.parameters.singleOrNull {
                ...
                "no constructor parameter for class ${kClass.simpleName} with name ${
                    columnDescriptor.name().lowercase()
                }"
            )
            ...
            val converter = if (annotation == null) {
                val clazz = (parameter.type.classifier as KClass<*>).javaObjectType
                converterRegistry.getConverterForColumn(clazz, columnDescriptor.jdbcType())
            } else {
                converterRegistry.getNamedConverter(annotation.name)
            }
        ...
        return map { row ->
            ...
            converters.forEachIndexed { index, parameterAndConvert ->
                val value = parameterAndConvert.converter.getObject(resultRow, index + 1)
    ...
    private fun <T : Any> verifyConstructor(clazz: KClass<T>): KFunction<T> {
        ...
        primaryConstructor.parameters.forEach {
            if (it.isOptional && it.type.isMarkedNullable)
                error("mapping a nullable type with default value does not make sense")
        }
    ...
    fun <T : Any> cursor(sql: String, clazz: KClass<T>, vararg params: Any?, batchSize: Int = 100): Flow<T> {
        ...
        return flow {
            ...
            val cursor = try {
                ...
                (sqlClient as SqlConnection).prepare(sql).coAwait().cursor(getTuple(params))
            } catch (e: Throwable) {
                throw ex.initCause(e)
            }
            try {
                cursor.read(batchSize).coAwait().mapRowsToObjects(clazz, converterRegistry, primaryConstructor).forEach {
                    emit(it)
                }
    ...
    suspend inline fun <reified T : Any> select(sql: String, vararg params: Any?): List<T> {
        return select(sql, T::class, params = params)
    }

    inline fun <reified T : Any> cursor(sql: String, vararg params: Any?, batchSize: Int = 1000): Flow<T> {
        return cursor(sql, T::class, params = params, batchSize)
    }

    suspend inline fun <reified T : Any> selectOne(sql: String, vararg params: Any?): T? {
        return select<T>(sql, params = params).singleOrNull()
    }

    suspend inline fun <reified T> selectPrimitive(sql: String, vararg params: Any?): List<T> {
        return selectPrimitive(sql, T::class, params = params)
    }
    ...
    suspend fun <T> selectPrimitive(sql: String, clazz: KClass<*>, vararg params: Any?): List<T> {
        ...
        try {
            ...
            val converter = converterRegistry.getConverterForColumn(
                clazz.java,
                result.columnDescriptors().single().jdbcType()
            )
            return result.map {
                val row = VertxResultRow(it)
                @Suppress("UNCHECKED_CAST")
                converter.getObject(row, 1) as T
            }
        } catch (e: Throwable) {
            throw ex.initCause(e)
        }
    ...
    suspend fun executeBatch(sql: String, params: List<List<Any?>>, batchSize: Int = 10000): List<RowSet<Row>> {
        ...
        try {
            @Suppress("SqlSourceToSinkFlow")
            val st = sqlClient.preparedQuery(sql)
    ...
    ) = sqlClient.preparedQuery(sql).execute(getTuple(params)).coAwait()

    private fun getTuple(params: Array<out Any?>): Tuple {
        return Tuple.from(params.map {
            if (it != null && it is Instant) {
                OffsetDateTime.ofInstant(it, ZoneId.of("UTC"))
            } else {
                it
            }
        })
    ...
    suspend inline fun <reified T> selectOnePrimitive(sql: String, vararg params: Any?): T? {
        return selectPrimitive<T>(sql, params = params).singleOrNull()
    }

    suspend inline fun <reified T> selectOneExistingPrimitive(sql: String, vararg params: Any?): T {
        return selectPrimitive<T>(sql, params = params).single()
    }
}
