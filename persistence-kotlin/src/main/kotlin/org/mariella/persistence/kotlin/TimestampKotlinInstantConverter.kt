...
object TimestampKotlinInstantConverter : Converter<Instant?> {
    override fun toString(value: Instant?): String {
        return value.toString()
    }

    override fun getObject(row: ResultRow, index: Int): Instant? {
        return row.getTimestamp(index)?.toInstant()?.toKotlinInstant()
    }
    ...
    override fun setObject(pv: ParameterValues, index: Int, value: Instant?) {
        if (value == null)
            ...
            pv.setTimestamp(index, Timestamp.from(value.toJavaInstant()))
    }

    override fun createLiteral(value: Any): Literal<Instant?> {
        return KotlinInstantLiteral(this, (value as Instant?))
    }
