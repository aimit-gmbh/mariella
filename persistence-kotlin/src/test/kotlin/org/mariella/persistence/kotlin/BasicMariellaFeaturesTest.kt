...
class BasicMariellaFeaturesTest : AbstractDatabaseTest() {
    ...
    @Test
    fun `can get autogenerated id on insert`() {
        assumeTrue(DATABASE_TYPE == DatabaseType.POSTGRES)
        runTest {
            val r = database.write {
                val mariella = mariella()
                val record = mariella.create<RecordEntity> {
                    it.type = "blabla"
                }
                val record1 = mariella.create<RecordEntity> {
                    it.type = "blabla1"
                }
                val record2 = mariella.create<RecordEntity> {
                    it.type = "blabla2"
                }
                mariella.flush()
                listOf(record.id, record1.id, record2.id)
            }
            expectThat(r).containsExactly(1, 2, 3)
        }
    }

    @Test
    fun `PostgresJoinedUpsertStatement works`() {
        assumeTrue(DATABASE_TYPE == DatabaseType.POSTGRES)
        runTest {
            val versionId = database.write {
                val context = mariella()
                val space = context.addExisting<Space>(TestData.TEST_SPACE.toKotlinUuid())
                val user = context.addExisting<UserEntity>(TestData.USER_SEPPI)
                ...
                val purge = context.create<Purge> {
                    it.space = space
                    it.comment = "my comment"
                    it.owner = user
                    it.revision = revision
                    it.createdAt = revision.createdAt.toKotlinInstant()
                    it.entityId = "entityId-1"
                    it.requestor = context.addExisting<UserEntity>(TestData.USER_KARL)
                    it.status = "hansi"
                }
            ...
            database.write {
                ...
                expectThat(purge.requestor).isNotNull()
    ...
    @Test
    fun `autogenerated IDs dont work for JDBC`() {
        assumeTrue(DATABASE_TYPE == DatabaseType.H2_MEM || DATABASE_TYPE == DatabaseType.H2)
        runTest {
            database.write {
                val mariella = mariella()
                mariella.create<RecordEntity> {
                    it.type = "blabla"
                }
                val thrown = expectThrows<DatabaseException> { mariella.flush() }.subject.unwrapOrThrow<DatabaseException>()
                expectThat(thrown.message!!).contains("only implemented for postgres")
            }
        }
    }
