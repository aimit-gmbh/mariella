...
public class JoinedUpdateStatement extends AbstractPersistorStatement {
    private final JoinedClassMapping classMapping;
    private final List<Column> columns;

    public JoinedUpdateStatement(JoinedClassMapping classMapping, Table table, List<Column> columns) {
    ...
    protected String getSqlString(BuildCallback buildCallback) {
        ...
        b.append(" UPDATE ").append(table.getQualifiedName()).append(" SET ");
        first = true;
        for (Column column : columns) {
            if (first)
                first = false;
